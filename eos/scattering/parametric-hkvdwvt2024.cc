...
namespace eos
{
    double HKVDWVT2024ScatteringAmplitudes::_calc_w(const double & s, const double & s0) const
    {
        if (s > s0)
    ...
    double HKVDWVT2024ScatteringAmplitudes::_calc_s(const complex<double> & z, const double & sp, const double & s0) const
    {
        if (s0 > sp)
    ...
    complex<double> HKVDWVT2024ScatteringAmplitudes::_calc_z(const double & s, const double & sp, const double & s0) const
    {
        if (s0 > sp)
    ...
    // S wave
    complex<double> HKVDWVT2024ScatteringAmplitudes::_omnes_S0(const double & s) const
    {
        const static thread_local OmnesInterpolation Om11_interpolation = {
                {
                    #include "svalues_om_S.hh"
                },
                {
                    #include "reom11_S.hh"
                },
        ...
        if (s <= 9.88873)
        {
            // TODO: Make Kaon contribution parameter
            return Om11_interpolation(s) + 2.0 / std::sqrt(3.0) * Om12_interpolation(s) * 0.5;
        }
        else
        {
            return (Om11_interpolation(9.88873) + 2.0 / std::sqrt(3.0) * Om12_interpolation(9.88873) * 0.5) * 9.88873 / s;
        }
    }

    // P wave
    double HKVDWVT2024ScatteringAmplitudes::_phase_P1(const double & s) const
    {
        const static thread_local PhaseInterpolation CHS_interpolation = {
                {
                    #include "svalues_CHS.hh"
                },
                {
                    #include "delvalues_CHS.hh"
                }
        };
    ...
    // D0 wave
    double HKVDWVT2024ScatteringAmplitudes::_phase_D0(const double & s) const
    {
    ...
    HKVDWVT2024ScatteringAmplitudes::HKVDWVT2024ScatteringAmplitudes(const Parameters & p, const Options &) :
        ...
        _f_phase_P1(std::bind(&HKVDWVT2024ScatteringAmplitudes::_phase_P1, this, std::placeholders::_1)),
        _f_phase_D0(std::bind(&HKVDWVT2024ScatteringAmplitudes::_phase_D0, this, std::placeholders::_1)),
        _omnes_P1(_intervals_P1, _f_phase_P1, 0.0),
        _omnes_D0(_intervals_D0, _f_phase_D0, 0.0)
    {
    }

    HKVDWVT2024ScatteringAmplitudes::~HKVDWVT2024ScatteringAmplitudes() = default;

    ScatteringAmplitudes<PPToPP> *
    HKVDWVT2024ScatteringAmplitudes::make(const Parameters & parameters, const Options & options)
    {
        return new HKVDWVT2024ScatteringAmplitudes(parameters, options);
    }

    QualifiedName
    HKVDWVT2024ScatteringAmplitudes::_par_name(const std::string & partial_wave, const std::string & par_name, unsigned idx) const
    {
        return QualifiedName(stringify(PiPiToPiPi::label) + "::" + partial_wave + "_" + par_name + "_" + stringify(idx) + "@GMKPRDEY2011");
    }

    QualifiedName
    HKVDWVT2024ScatteringAmplitudes::_par_name(const std::string & partial_wave, const std::string & par_name) const
    {
        return QualifiedName(stringify(PiPiToPiPi::label) + "::" + partial_wave + "_" + par_name + "@GMKPRDEY2011");
    }

    complex<double> HKVDWVT2024ScatteringAmplitudes::scattering_amplitude(const double & s, const unsigned & l, const IsospinRepresentation & i) const
    {
        if (s <= 4 * _mPi * _mPi)
        {
            return 0.0;
    ...
    complex<double> HKVDWVT2024ScatteringAmplitudes::omnes_factor(const double & s, const unsigned & l, const IsospinRepresentation & i) const
    {
        if ((l == 0) && (i == IsospinRepresentation::zero))
        {
            //std::cerr << "S-Wave called" << _omnes_S0(s) << std::endl;
            return _omnes_S0(s);
        }
        else if ((l == 1) && (i == IsospinRepresentation::one))
        {
            //std::cerr << "P-Wave called" << _omnes_P1(s) << std::endl;
            return _omnes_P1(s);
        }
        else if ((l == 2) && (i == IsospinRepresentation::zero))
        {
    ...
    complex<double> HKVDWVT2024ScatteringAmplitudes::isospin_breaking(const double & s, const unsigned & l, const IsospinRepresentation & i) const
    {
    ...
    complex<double> HKVDWVT2024ScatteringAmplitudes::omnes_outer_function(const double & s, const double & sp, const double & s0, const unsigned & npoints, const unsigned & l, const IsospinRepresentation & i) const
    {
    ...
    Diagnostics
    HKVDWVT2024ScatteringAmplitudes::diagnostics() const
    {
        Diagnostics results;
    ...
    const std::set<ReferenceName> HKVDWVT2024ScatteringAmplitudes::references
    {
        "HKVDWVT:2024A"_rn
    };

    const std::vector<OptionSpecification> HKVDWVT2024ScatteringAmplitudes::options
    {
    };

    std::vector<OptionSpecification>::const_iterator
    HKVDWVT2024ScatteringAmplitudes::begin_options()
    {
        return options.cbegin();
    }

    std::vector<OptionSpecification>::const_iterator
    HKVDWVT2024ScatteringAmplitudes::end_options()
    {
        return options.cend();
    }
}
