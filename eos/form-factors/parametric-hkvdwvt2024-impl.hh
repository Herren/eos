...
#ifndef EOS_GUARD_EOS_FORM_FACTORS_PARAMETRIC_HKVDWVT2024_IMPL_HH
#define EOS_GUARD_EOS_FORM_FACTORS_PARAMETRIC_HKVDWVT2024_IMPL_HH 1
...
namespace eos
{
    ...
    HKVDWVT2024FormFactorTraits<Process_, PToPP2>::resonance_0m_names
    {
    ...
    template <typename Process_>
    const std::map<std::tuple<QuarkFlavor, QuarkFlavor>, std::string>
    HKVDWVT2024FormFactorTraits<Process_, PToPP2>::resonance_1m_names
    {
        { std::make_tuple(QuarkFlavor::bottom, QuarkFlavor::up), "mass::B_u^*@BSZ2015" },
        { std::make_tuple(QuarkFlavor::bottom, QuarkFlavor::down), "mass::B_d^*@BSZ2015" },
        { std::make_tuple(QuarkFlavor::bottom, QuarkFlavor::strange), "mass::B_s^*@BSZ2015" },
    ...
    template <typename Process_>
    const std::map<std::tuple<QuarkFlavor, QuarkFlavor>, std::string>
    HKVDWVT2024FormFactorTraits<Process_, PToPP2>::resonance_1p_names
    {
    ...
    template<typename Process_>
    HKVDWVT2024FormFactors<Process_, PToPP2>::HKVDWVT2024FormFactors(const Parameters & p, const Options &) :
        _traits(HKVDWVT2024FormFactorTraits<Process_, PToPP2>(p)),
        _mP1(_traits.m_P1),
        _mP2(_traits.m_P2),
        _mP3(_traits.m_P3),
    ...
    template<typename Process_>
    HKVDWVT2024FormFactors<Process_, PToPP2>::~HKVDWVT2024FormFactors() = default;

    template<typename Process_>
    FormFactors<PToPP2> *
    HKVDWVT2024FormFactors<Process_, PToPP2>::make(const Parameters & parameters, const Options & options)
    {
        return new HKVDWVT2024FormFactors(parameters, options);
    }

    template<typename Process_>
    QualifiedName
    HKVDWVT2024FormFactors<Process_, PToPP2>::_par_name(const std::string & ff_name, unsigned wave, unsigned idx) const
    {
        return QualifiedName(stringify(Process_::label) + "::a^" + ff_name + "_" + stringify(wave) + "_" + stringify(idx) + "@HKVDWVT2024");
    }

    template<typename Process_>
    double
    HKVDWVT2024FormFactors<Process_, PToPP2>::_phi(const int & l, const double & s, const double & q2, const double & q2p, const double & chi,
                        const unsigned a, const unsigned b, const double N) const
    {
        ...
        //const double y = _traits.calc_y(s, sin, _traits.s0);
        ...
        //const double kinematic_sp = power_of<2>(_mP2 + _mP3);
        //const double kinematic_sm = power_of<2>(_mP2 - _mP3);

        // set Q^2 to 0
        //const double s_term = 1 / (2 * (sin + std::sqrt(sin) * std::sqrt(sin - s)) - s);
        //const double lambda_s_term = power_of<2>(std::sqrt(sin - s) + std::sqrt(sin - kinematic_sp))*power_of<2>(std::sqrt(sin - s) + std::sqrt(sin - kinematic_sm));
        ...
        //const double sqrtjac_s = std::sqrt(4 * (y + 1) * (_traits.s0 - sin) / power_of<3>(y - 1));
        const double sqrtjac_q2 = std::sqrt(4 * (1 + z) * (_traits.q20 - q2p) / power_of<3>(z - 1));

        //return norm * sqrtjac_s * sqrtjac_q2 * pow(s_term, (2 * l + 1 - a) * 0.5) * pow(lambda_s_term, (2 * l + 1) * 0.25) * pow(_mP1 * _mP1 - s, b) * pow(lambda_q2_term, (2 * l + 1 - 2 * c) * 0.25) * pow(q2_term, d * 0.5);
    ...
    template<typename Process_>
    inline double
    HKVDWVT2024FormFactors<Process_, PToPP2>::_phi_g(const double & q2, const double & s, const unsigned & l) const
    {
        if (l > 0)
        {
            return _phi(l, s, q2, _traits.q2p_v, Process_::chi_1m_v, 0, 4, l * (l + 1) / 12.0);
        }
        else
        {
            return 1.0;
    ...
    template<typename Process_>
    inline double
    HKVDWVT2024FormFactors<Process_, PToPP2>::_phi_f(const double & q2, const double & s, const unsigned & l) const
    {
        if (l > 0)
        {
    ...
    template<typename Process_>
    inline double
    HKVDWVT2024FormFactors<Process_, PToPP2>::_phi_F1(const double & q2, const double & s, const unsigned & l) const
    {
        if (l > 0)
        {
    ...
    template<typename Process_>
    inline double
    HKVDWVT2024FormFactors<Process_, PToPP2>::_phi_F2(const double & q2, const double & s, const unsigned & l) const
    {
        if (l > 0)
        {
    ...
    template <typename Process_>
    complex<double>
    HKVDWVT2024FormFactors<Process_, PToPP2>::v_perp(const double & q2, const double & s, const unsigned & l, const bool & iso) const
    {
        ...
        const complex<double> y             = _traits.calc_y(complex<double>(s), complex<double>(_traits.sin), complex<double>(_traits.s0));
    ...
    template <typename Process_>
    complex<double>
    HKVDWVT2024FormFactors<Process_, PToPP2>::a_par(const double & q2, const double & s, const unsigned & l, const bool & iso) const
    {
        const double isospin_factor = (iso) ? std::sqrt(Process_::eta_2[l]) : std::sqrt(Process_::eta_1[l]);
        if ( isospin_factor == 0.0 )
                return 0.0;
    ...
    template <typename Process_>
    complex<double>
    HKVDWVT2024FormFactors<Process_, PToPP2>::a_0(const double & q2, const double & s, const unsigned & l, const bool & iso) const
    {
        const double isospin_factor = (iso) ? std::sqrt(Process_::eta_2[l]) : std::sqrt(Process_::eta_1[l]);
        if ( isospin_factor == 0.0 )
                return 0.0;
    ...
    template <typename Process_>
    complex<double>
    HKVDWVT2024FormFactors<Process_, PToPP2>::a_t(const double & q2, const double & s, const unsigned & l, const bool & iso) const
    {
        const double isospin_factor = (iso) ? std::sqrt(Process_::eta_2[l]) : std::sqrt(Process_::eta_1[l]);
        if ( isospin_factor == 0.0 )
                return 0.0;
    ...
    template <typename Process_>
    double
    HKVDWVT2024FormFactors<Process_, PToPP2>::unitarity_integrand_0m(const double & s, const unsigned & l, const bool & iso) const 
    {
        double res = 0.0;
    ...
    template <typename Process_>
    double
    HKVDWVT2024FormFactors<Process_, PToPP2>::unitarity_integrand_1p(const double & s, const unsigned & l, const bool & iso) const 
    {
        double res_f = 0.0;
        double res_F1 = 0.0;
        ...
        /*if (l != 0)
        {
            kinpref_F1 *= power_of<2>((_mP1 * _mP1 - s) / s);
        }*/
    ...
    template <typename Process_>
    double
    HKVDWVT2024FormFactors<Process_, PToPP2>::unitarity_integrand_1m(const double & s, const unsigned & l, const bool & iso) const 
    {
        double res = 0.0;
    ...
    template <typename Process_>
    Diagnostics
    HKVDWVT2024FormFactors<Process_, PToPP2>::diagnostics() const
    {
        Diagnostics results;

        results.add({ _traits.calc_y(4 * 0.135 * 0.135,  _traits.sin, _traits.s0), "y(s = 4*0.135^2)" });
        results.add({ _traits.calc_y(0.1,                _traits.sin, _traits.s0), "y(s = 0.1)"       });
...

        {
            ...
            //results.add({ p3,              "p_3(z = 0.0, s = 0.1)" });
            //results.add({ p4,              "p_4(z = 0.0, s = 0.1)" });
            //results.add({ p5,              "p_5(z = 0.0, s = 0.1)" });
        }

        {
            ...
            //results.add({ p3,              "p_3(z = z(q2 = 10, s = 0.1))" });
            //results.add({ p4,              "p_4(z = z(q2 = 10, s = 0.1))" });
            //results.add({ p5,              "p_5(z = z(q2 = 10, s = 0.1))" });
        }

        {
            const auto & [p0, p1, p2] = _traits.threshold_improved_polynomials(_traits.calc_y(0.5, _traits.sin, _traits.s0), 0);
        ...
        {
            const auto & [p0, p1, p2] = _traits.threshold_improved_polynomials(_traits.calc_y(0.5, _traits.sin, _traits.s0), 1);
        ...
        {
            const auto & [p0, p1, p2] = _traits.threshold_improved_polynomials(_traits.calc_y(0.5, _traits.sin, _traits.s0), 2);
    ...
    template<typename Process_>
    const std::set<ReferenceName> HKVDWVT2024FormFactors<Process_, PToPP2>::references
    {
        "HKVDWVT:2024A"_rn
    };

    template<typename Process_>
    const std::vector<OptionSpecification> HKVDWVT2024FormFactors<Process_, PToPP2>::options
    {
    };

    template<typename Process_>
    std::vector<OptionSpecification>::const_iterator
    HKVDWVT2024FormFactors<Process_, PToPP2>::begin_options()
    {
    ...
    template<typename Process_>
    std::vector<OptionSpecification>::const_iterator
    HKVDWVT2024FormFactors<Process_, PToPP2>::end_options()
    {
        return options.cend();
    }
}
